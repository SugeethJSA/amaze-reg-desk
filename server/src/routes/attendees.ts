...
attendeesRouter.get("/", requireAuth, async (req, res, next) => {
  try {
    ...
    const result = await pool.query(
      `SELECT id, external_ref, name, email, phone, college, department, registered_on_spot, created_at
         FROM attendees
...
attendeesRouter.post("/", requireAuth, requireAdmin, async (req, res, next) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO attendees (external_ref, name, email, phone, college, department, registered_on_spot)
       ...
       SET name = EXCLUDED.name,
           ...
           updated_at = now()
       RETURNING *`,
      [input.externalRef, input.name, input.email.toLowerCase(), input.phone, input.college, input.department]
    );
